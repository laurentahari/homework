...
def task0():
    print ('Lauren Tahari')
    print ('My expected score is a 60/60')
...
def name_counts(names):
    ...
    for line in names:
        if line in counts:
            ...
            counts[line] = 1
    sortedlist = sorted(counts)
    sortedlist2 = sorted(sortedlist, key= lambda x: counts[x], reverse = True)
    sortedlist3 = []
    for key in sortedlist2:
        sortedlist3.append((key, counts[key]))
    return sortedlist3
...
def build_acronym(ls):
    firstcharacters = ''
    for word in ls:
        firstcharacters += word[0]
    return firstcharacters.upper()
